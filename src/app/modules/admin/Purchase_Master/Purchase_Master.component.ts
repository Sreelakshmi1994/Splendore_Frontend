...
this.issLoading=true;
debugger
 this.Purchase_Master_Service_.Search_Purchase_Master(look_In_Date_Value,moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'),ClientAccount,Voucher_No_search_).subscribe(Rows => {
    debugger
    this.Purchase_Master_Data=Rows[0];
 this.Total_Entries=this.Purchase_Master_Data.length;
    if(this.Purchase_Master_Data.length==0)
...
Get_Item_Group(event: any) {
    ...
    this.issLoading = true;
    debugger
    this.Purchase_Master_Service_.Search_Item_Group1('1,3',Value).subscribe(
        (Rows) => {
            debugger
            if (Rows != null) {
