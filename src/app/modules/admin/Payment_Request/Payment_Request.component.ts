...
// import { Item_Group } from '../../../models/Item_Group';
 @Component({
selector: 'app-Payment_Request',
templateUrl: './Payment_Request.component.html',
styleUrls: ['./Payment_Request.component.css'],
providers: [
        { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
        { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },],
})
export class Payment_RequestComponent implements OnInit {
...
Final_Amounts()
{
    ...
    for(var i = 0; i< this.Payment_Request_Details_Data.length ; i++)
    {      
        AMT=Number(this.Payment_Request_Details_Data[i].TotalAmount);;
        Tot_Amount = Number(Tot_Amount) + Number(AMT);
    }
    this.Payment_Request_.NetTotal = Number(Tot_Amount.toFixed(2));
    debugger;
...
dialogRef.afterClosed().subscribe(result =>
    ...
    if(result=='Yes')
        ...
        this.Total_Amounts=Number(this.Total_Amounts)-Number(this.Payment_Request_Data[index].TotalAmount);
...
this.issLoading=true;
debugger
 this.Payment_Request_Service_.Search_Payment_Request(look_In_Date_Value,moment(this.Search_FromDate).format('YYYY-MM-DD'), moment(this.Search_ToDate).format('YYYY-MM-DD'),ClientAccount,Voucher_No_search_).subscribe(Rows => {
 this.Payment_Request_Data=Rows[0];
 debugger
 this.Total_Entries=this.Payment_Request_Data.length;
 debugger
 for(var i=0;i<this.Payment_Request_Data.length;i++)
 {
         this.Total_Amounts=Number(this.Total_Amounts)+Number(this.Payment_Request_Data[i].TotalAmount);
 }
    if(this.Payment_Request_Data.length==0)
    {
    const dialogRef = this.dialogBox.open( DialogBox_Component, {panelClass:'Dialogbox-Class',data:{Message:'No Details Found',Type: "3" }});
...
{    
    ...
    console.log(' this.Payment_Request_Details_.TotalAmount: ',  this.Payment_Request_Details_.TotalAmount);
    debugger;
...
//   else  if(this.Payment_Request_.InvoiceNo==undefined || this.Payment_Request_.InvoiceNo==null ||  this.Payment_Request_.InvoiceNo=="")
//     {
...
//     }
    ...
    else if(this.Payment_Request_.Description==undefined || this.Payment_Request_.Description==null || this.Payment_Request_.Description=='' )
  {
    ...
    this.issLoading=true;      
    console.log('this.Payment_Request_: ', this.Payment_Request_);
   this.Payment_Request_Service_.Save_Payment_Request(this.Payment_Request_,this.Doc_Photo,
    this.ImageFile_Doc,
...
this.Payment_Request_Print=true; 
    ...
    console.log('his.Payment_Request_: ', this.Payment_Request_);
    this.Payment_Request_=Object.assign({},Payment_Request_e);
}
